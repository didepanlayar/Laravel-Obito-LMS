...
class CourseService
{
    public function enrollUser(Course $course)
    {
        ...
        if (!$course->courseStudents()->where('user_id', $user->id)) {
    ...
    public function getFirstSectionAndContent(Course $course)
    {
        ...
        $firsContentId = $firstSectionId ? $course->courseSections()->find($firstSectionId)->sectionContents()->orderBy('id')->value('id') : null;

        return [
            'firstSectionId' => $firstSectionId,
            'firstContentId' => $firsContentId,
        ];
    }
